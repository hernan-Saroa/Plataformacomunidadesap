...
export interface ProcesoAuditable {
  ...
  // EVALUACIÓN INICIAL (7 Preguntas - Sistema Rápido)
  evaluacionInicial: EvaluacionInicial;
  
  // EVALUACIÓN DAFP COMPLETA (Sistema Oficial - Opcional)
...
/**
 * Evaluación Inicial Rápida (7 Preguntas)
 * Sistema simplificado para evaluación rápida de riesgos
 */
export interface EvaluacionInicial {
  // Cuestionario de 7 preguntas (0-4 puntos cada una)
  ...
  scoreRiesgo: number; // 0-100 (suma/28 * 100)
  nivelRiesgo: 'Crítico' | 'Alto' | 'Medio' | 'Bajo';
  frecuenciaSugerida: 'Anual' | 'Semestral' | 'Bienal' | 'Trienal';
...
/**
 * Evaluación DAFP Completa
 * Sistema oficial basado en la Guía DAFP Versión 4 (Julio 2020)
 */
export interface EvaluacionDafpCompleta {
  ...
  riesgosBajos: number;
  totalRiesgos: number; // Calculado automáticamente
  
  // Factores adicionales
  requerimientoComite: boolean; // Comité de Auditoría o Dirección
  requerimientoEntesReg: boolean; // Entes Reguladores
  
  // Última auditoría
  fechaUltimaAuditoria: string | null;
  resultadoUltimaAuditoria: 'ADECUADO' | 'INADECUADO' | 'SIN_AUDITORIA';
  diasDesdeUltimaAuditoria: number | null; // Calculado
  
  // Resultados calculados (Motor DAFP)
  ponderacionRiesgo: 'EXTREMO' | 'ALTO' | 'MODERADO' | 'BAJO' | 'MUY_BAJO' | null;
  planRotacion: string | null; // "1 año", "2 años", etc.
  planRotacionDias: number | null; // 360, 720, 1080, 1440, 1800
  decisionRotacion: 'INCLUIR' | 'NO_INCLUIR' | null;
  decisionFinal: 'INCLUIR_PLAN_ANUAL' | 'INCLUIR_AUDITORIA_POSTERIOR';
  motivoInclusion: string; // Justificación automática
  
  // Detalle de cálculos (trazabilidad)
  detalleCalculo: {
...
/**
 * LEGACY: EvaluacionRiesgo (Mantener para compatibilidad)
 * @deprecated Usar EvaluacionInicial o EvaluacionDafpCompleta
 */
