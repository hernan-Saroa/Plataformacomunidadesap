/**
 * MODAL GESTIONAR PLANTILLAS OFICIO
 * Sistema completo para gestionar plantillas de un tipo de oficio
...
export function ModalGestionarPlantillasOficio({ 
  tipoOficio, 
  onGuardar, 
  onCerrar 
}: ModalGestionarPlantillasOficioProps) {
  // Inicializar plantillas desde tipoOficio.plantilla
  const [plantillas, setPlantillas] = useState<PlantillaArchivo[]>(() => {
    if (tipoOficio.plantilla) {
      return [tipoOficio.plantilla];
    }
    return [];
  });
  ...
  useEffect(() => {
    if (tipoOficio.plantilla) {
      setPlantillas([tipoOficio.plantilla]);
    } else {
      setPlantillas([]);
    }
  }, [tipoOficio]);
  ...
  return (
    <>
          ...
          >
            ...
            >
              <div>
                ...
                <p className="text-sm mt-0.5 text-purple-100">
                  {tipoOficio.nombre} · {plantillas.length} plantilla{plantillas.length !== 1 ? 's' : ''}
                </p>
              </div>
              <button
                onClick={onCerrar}
            ...
            <div className="p-5 overflow-y-auto max-h-[calc(90vh-200px)]">
              <div className="space-y-4">
                {/* Información del Tipo de Oficio */}
                <div className="bg-purple-50 border-l-4 border-purple-500 p-3 rounded">
                  <div className="flex items-start gap-2">
                    <Info className="w-4 h-4 text-purple-700 flex-shrink-0 mt-0.5" />
                    <div className="text-xs text-purple-900">
                      <p className="font-semibold mb-1">Sobre este tipo de oficio:</p>
                      <p>{tipoOficio.descripcion}</p>
                    </div>
                  </div>
                </div>
                ...
                {plantillas.length === 0 ? (
                  <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-12 text-center">
                    ...
                    <p className="text-sm text-gray-600 mb-3">
                      No hay plantillas configuradas para este tipo de oficio
                    </p>
                    <button
                      onClick={() => setModalAgregarPlantilla(true)}
                        ...
                        >
                          <div className="flex items-start gap-4">
                            ...
                            <div className={`w-12 h-12 rounded-lg flex items-center justify-center flex-shrink-0 ${
                              plantilla.activo ? 'bg-purple-100' : 'bg-gray-200'
                            }`}>
                              <File className={`w-6 h-6 ${plantilla.activo ? 'text-purple-600' : 'text-gray-500'}`} />
                            </div>
      ...
      {modalAgregarPlantilla && (
        <ModalFormularioPlantillaOficio
...
// ============ MODAL FORMULARIO PLANTILLA OFICIO ============

interface ModalFormularioPlantillaOficioProps {
  isOpen: boolean;
  onClose: () => void;
...
function ModalFormularioPlantillaOficio({ 
  isOpen, 
  onClose, 
  onGuardar, 
  plantillaEdicion 
}: ModalFormularioPlantillaOficioProps) {
        ...
        >
          ...
          >
            <div>
              ...
              <p className="text-sm mt-0.5 text-purple-100">
                Configura el archivo de plantilla Word
              </p>
            </div>
            <button
              onClick={onClose}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 text-sm border rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition-all ${
                    errores.nombre ? 'border-red-500' : 'border-gray-300'
                  }`}
                  placeholder="Ej: Oficio de Solicitud de Información - Versión 2024"
                  disabled={guardando}
                />
                {errores.nombre && (
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className={`w-full px-3 py-2 text-sm border rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition-all resize-none ${
                ...
                {!archivo && !archivoExistente ? (
                  ...
                  >
                    <Upload className={`w-10 h-10 mx-auto mb-3 ${
                      dragging ? 'text-purple-500' : errores.archivo ? 'text-red-500' : 'text-gray-400'
                    }`} />
                    <p className="text-sm font-semibold text-gray-700 mb-1">
                      Arrastra el archivo aquí o haz clic para seleccionar
                    </p>
                    <p className="text-xs text-gray-600">
                      Formatos permitidos: .doc, .docx, .dotx, .rtf (Máximo 10 MB)
                    </p>
                  </div>
                ) : (
