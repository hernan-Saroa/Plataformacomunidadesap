...
import { BadgeNomenclatura } from './components/BadgeNomenclatura'; // ✅ NUEVO
import { generarNomenclatura, previsualizarNomenclatura } from './utils/nomenclaturaDocumentos'; // ✅ NUEVO
import { ModalSubirDocumento } from './ModalSubirDocumento'; // ✅ NUEVO: Para formulario completo de evidencias
...
export function ModalGestionAutos({ proceso, onClose, onCrearAuto }: ModalAutosProps) {
  ...
  // ✅ NUEVO: Obtener vista previa de la siguiente nomenclatura
  const proximaNomenclatura = previsualizarNomenclatura('AUTO');

  // Mock data de autos CON NOMENCLATURA ÚNICA ✅
  const autos = [
    {
      id: 'a1',
      nomenclatura: 'AUT-041-2025', // ✅ Nomenclatura única
      tipo: 'Apertura',
      fecha: '2025-01-08',
      firmado: true,
      notificado: true,
    ...
    {
      id: 'a2',
      nomenclatura: 'AUT-002-2025', // ✅ Nomenclatura única
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        {/* ✅ NUEVO: Badge de Nomenclatura Única */}
                        <BadgeNomenclatura 
                    ...
                    <div className="flex gap-1">
                      ...
                      <Button 
                        ...
                        onClick={(e) => {
                          ...
                          link.href = '#';
                          link.download = `${auto.nomenclatura}.pdf`;
...
export function ModalGestionEvidencias({ proceso, onClose, onSubirEvidencia }: ModalEvidenciasProps) {
  const [visorDocumento, setVisorDocumento] = useState<{ show: boolean; documento: any }>({ show: false, documento: null });
  const [mostrarModalSubir, setMostrarModalSubir] = useState(false); // ✅ NUEVO: Controla apertura del ModalSubirDocumento
  ...
  const [evidenciasLocal, setEvidenciasLocal] = useState([
    {
      id: 'e1',
      nomenclatura: 'EV-001-2025', // ✅ Nomenclatura única
    ...
    {
      id: 'e2',
      nomenclatura: 'EV-002-2025', // ✅ Nomenclatura única
    ...
    {
      id: 'e3',
      nomenclatura: 'EV-003-2025', // ✅ Nomenclatura única
  ...
  // ✅ Estados de carga eliminados - Ya no se usan (ahora usamos ModalSubirDocumento)
    ...
    >
      ...
      >
        ...
        <div className="p-6 overflow-y-auto" style={{ maxHeight: 'calc(90vh - 260px)' }}>
          ...
          {/* Botón de Subir - ✅ MODIFICADO: Ahora abre modal con formulario completo */}
          <div className="relative">
            <Card
              onClick={() => setMostrarModalSubir(true)}
              className="mt-4 p-8 border-2 border-dashed cursor-pointer transition-all hover:bg-gray-50 border-gray-300"
            >
                <div className="text-center">
                  <Upload 
                  ...
                  <p className="text-sm text-gray-600">
                    Click para abrir formulario de carga
                  </p>
                  <p className="text-xs text-gray-500 mt-2">
                    ✅ Con campos de nombre y descripción
                  </p>
                </div>
              </Card>
          </div>
      ...
      {/* ✅ NUEVO: Modal Subir Documento con Nombre y Descripción */}
      <ModalSubirDocumento
        ...
        onConfirm={(documentos) => {
            ...
            const nuevasEvidencias = documentos.map((doc, index) => ({
              ...
              nomenclatura: generarNomenclatura('EVIDENCIA'),
              nombre: doc.nombreEvidencia, // ✅ Nombre ingresado por el usuario
              tipo: doc.tipoDocumento,
              fecha: new Date().toISOString().split('T')[0],
              tamaño: doc.archivo ? `${(doc.archivo.size / 1024 / 1024).toFixed(2)} MB` : '0 MB',
              categoria: doc.tipoDocumento === 'Prueba Documental' ? 'Documental' : 'Digital',
              descripcion: doc.descripcion // ✅ Descripción ingresada por el usuario
            }));
            
            setEvidenciasLocal(prev => [...nuevasEvidencias, ...prev]);
