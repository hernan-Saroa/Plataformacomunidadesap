...
import { ResponsiveTable, createColumn, type TableColumn } from '../ui/ResponsiveTable';
...
export function AuditLogTable({ events, onEventClick, searchQuery = '' }: AuditLogTableProps) {
  ...
  const columns: TableColumn<AuditEvent>[] = [
    ...
    createColumn<AuditEvent>({
      ...
      header: 'Acciones',
      align: 'center',
      hideOnMobile: true, // Botón en footer de card mobile
      render: (event) => (
        <button
  ...
  const renderMobileCard = (event: AuditEvent) => {
    const severityConfig = getSeverityConfig(event.severity);
    const statusConfig = getStatusConfig(event.status);
    const SeverityIcon = severityConfig.icon;
    const StatusIcon = statusConfig.icon;
