          ...
          {buttons.map((button, idx) => {
            ...
            return (
              <button
                ...
                className={`px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg flex items-center gap-1 sm:gap-2 text-xs sm:text-sm transition-all ${button.className || defaultClassName
        ...
        <div className="flex items-center gap-1 p-1 rounded-lg w-full sm:w-auto overflow-x-auto" style={{ background: '#F3F4F6' }}>
          {toggleView.options.map((option, idx) => (
            <button
              ...
              className={`flex-1 sm:flex-none px-2 sm:px-3 py-1.5 sm:py-2 rounded-md text-xs sm:text-sm font-semibold flex items-center justify-center gap-1 sm:gap-1.5 transition-all whitespace-nowrap ${toggleView.current === (option.value || option.label.toLowerCase())
                  ? 'bg-white shadow-sm'
                  : 'hover:bg-gray-200'
                }`}
              style={{
