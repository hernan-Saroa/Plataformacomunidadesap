    ...
    <Card className="bg-white border border-gray-200 hover:shadow-md transition-all">
      ...
      <div className={`${isMobile ? 'p-2.5' : 'p-3'}`}>
        <div className="flex items-start justify-between mb-2">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-gray-600 truncate">{requerimiento.organo}</p>
            </div>
          </div>
          <span className="text-lg" title="Órgano de control">{getOrganoIcon(requerimiento.organo)}</span>
        </div>
          ...
          <div className="flex items-center gap-2">
            <Avatar className={`${isMobile ? 'w-5 h-5' : 'w-6 h-6'} flex-shrink-0`}>
              <AvatarFallback className="text-xs" style={{ background: '#E0EDFF', color: '#003DA5' }}>
                {requerimiento.responsable.slice(0, 2).toUpperCase()}
              </AvatarFallback>
            </Avatar>
        ...
        <div className="mb-2 p-2 rounded-lg" style={{ backgroundColor: '#F0F7FF', border: '1px solid #BFDBFE' }}>
          ...
          <p className="text-xs text-gray-500">📅 {requerimiento.fechaRadicacion.toLocaleDateString('es-CO')}</p>
        </div>
