...
export function ModuloJuzgamientoDisciplinarioV3() {
  ...
  // ✅ ESTADO COMPLETAMENTE VACÍO - PRODUCCIÓN LIMPIA
  const [procesos, setProcesos] = useState<ProcesoDisciplinario[]>([]);

  // ✅ DEBUG: Verificar que los datos mock están vacíos
  useEffect(() => {
    console.log('🧹 LIMPIEZA TOTAL EJECUTADA');
    console.log('📊 JUZGAMIENTO DISCIPLINARIO - Datos cargados:', procesosDisciplinariosMock.length, 'procesos');
    console.log('📊 Estado local de procesos:', procesos.length, 'procesos');
    if (procesosDisciplinariosMock.length > 0) {
      console.warn('⚠️ ADVERTENCIA: Todavía hay datos mock en procesosDisciplinariosMock');
      console.warn('Datos encontrados:', procesosDisciplinariosMock);
    } else {
      console.log('✅ CONFIRMADO: procesosDisciplinariosMock está vacío');
      console.log('✅ CONFIRMADO: Estado local está vacío');
    }
  }, []);

  // ✅ Estado para items archivados/eliminados - VACÍO PARA PRODUCCIÓN
