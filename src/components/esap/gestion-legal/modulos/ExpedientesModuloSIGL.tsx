...
import {
  ...
  File as FileIcon, FolderCheck, FileCheck, Scale, Gavel, FileQuestion,
  BarChart3
} from 'lucide-react';
...
interface Documento {
  ...
  autor: string;
  file?: File; // Propiedad para archivos cargados localmente
}
...
export function ExpedientesModuloSIGL() {
  ...
  useEffect(() => {
    const fetchExpedientes = async () => {
      try {
        ...
        const expedientesUnificados: Expediente[] = [];

        // Helper to normalize data
        const mapToExpediente = (source: any, tipo: TipoProceso, responsableKey: string, idKey: string = 'id', nombreKey: string = 'nombreProceso', radicadoKey: string = 'radicado') => {
          // Helper local safe string extractor
          const getSafeString = (obj: any, keys: string[] = ['nombre', 'email', 'id']) => {
          ...
          // Smart Radicado Extraction with fallbacks
          // Explicitly include the dynamic idKey and ensure unique checks
          const uniqueKeys = new Set([radicadoKey, 'radicado', 'codigo', 'numero', 'radicadoRequerimiento', 'id', '_id', 'consecutivo', idKey]);
          const keysToCheck = Array.from(uniqueKeys);

          let rad = 'S/N';
          const RADICADO_REGEX = /^[A-Z]+(-[A-Z0-9]+)+$/i; // Detects patterns like PD-2025-001, CJ-2025-01

          // 1. Check logical keys first
          for (const key of keysToCheck) {
            const val = getSafeString(source[key]);
            if (val && val !== 'S/N' && val !== 'undefined' && String(val).trim() !== '') {
              rad = val;
              break;
            }
          }

          // 2. Ultimate Fallback: Force ID if still S/N
          if ((rad === 'S/N' || !rad) && (source[idKey] || source['_id'])) {
            rad = String(source[idKey] || source['_id']);
          }

          // 3. Brute Force Regex Search (Panic Mode)
          if (rad === 'S/N' || (rad.length > 20 && !rad.includes(' '))) {
            const values = Object.values(source);
            for (const v of values) {
              if (typeof v === 'string' && RADICADO_REGEX.test(v) && v.length < 20) {
                rad = v;
                break;
              }
            }
          ...
          if (rad === 'S/N') {
            const keysStr = Object.keys(source).join(',');
            console.warn(`⚠️ Expediente sin radicado detectado. Keys: ${keysStr}`, source);
          }

          let resp = getSafeString(source[responsableKey]) || 'Por Asignar';
          let procName = getSafeString(source[nombreKey]) || `${tipo} - ${rad}`;

          return {
            id: source[idKey],
            radicado: rad,
            nombreProceso: procName,
            tipoProceso: tipo,
            fechaInicio: source.fechaRadicacion || source.createdAt || new Date().toISOString().split('T')[0],
            fechaActualizacion: source.updatedAt || new Date().toISOString().split('T')[0],
            estado: (source.estado === 'CERRADO' ? 'FINALIZADO' : (source.estado === 'ACTIVO' ? 'ACTIVO' : 'EN_PROCESO')) as any,
            responsable: resp,
            totalDocumentos: source.documentos?.length || 0,
            documentos: []
          };
        };

        const procesadosIds = new Set<string>();
        const agregarUnico = (item: any, tipo: TipoProceso, respKey: string, idKey: string = 'id', nomKey: string = 'nombreProceso', radKey: string = 'radicado') => {
          if (!item) return;
          const id = item[idKey];
          if (id && !procesadosIds.has(id)) {
            procesadosIds.add(id);
            expedientesUnificados.push(mapToExpediente(item, tipo, respKey, idKey, nomKey, radKey));
          }
        };

        if (Array.isArray(juzgamiento)) juzgamiento.forEach((j: any) => agregarUnico(j, 'JUZGAMIENTO', 'abogadoAsignado', 'id', 'investigado', 'id'));
        if (Array.isArray(asesorias)) asesorias.forEach((a: any) => agregarUnico(a, 'ASESORIA', 'abogadoAsignado', 'id', 'asunto', 'codigo'));
        if (Array.isArray(coactivos)) coactivos.forEach((c: any) => agregarUnico(c, 'PROCESOS_COACTIVOS', 'funcionario', 'id', 'deudor', 'id'));
        if (Array.isArray(organos)) organos.forEach((o: any) => agregarUnico(o, 'ORGANOS_CONTROL', 'responsable', 'id', 'enteControl', 'radicadoRequerimiento'));

        // Procesar defensa al final para evitar que datos genéricos sobrescriban los específicos
        if (Array.isArray(defensa)) defensa.forEach((e: any) => agregarUnico(e, 'DEFENSA_JUDICIAL', 'abogadoSustanciador', 'id', 'nombreProceso', 'radicado'));

        setExpedientes(expedientesUnificados);
      } catch (error) {
        console.error("Error cargando expedientes:", error);
        toast.error("Error al cargar expedientes del sistema");
        // Fallback to Mocks if fetch fails completely to avoid empty screen crash if API down
  ...
  const handleDocumentoCargado = (expedienteId: string, nuevoDoc: Documento) => {
    setExpedientes(prev => prev.map(exp => {
      if (exp.id === expedienteId) {
        return {
          ...exp,
          totalDocumentos: exp.totalDocumentos + 1,
          documentos: [...exp.documentos, nuevoDoc]
        };
      }
      return exp;
    }));
        ...
        >
          {vistaActiva === 'expedientes' && (
            <VistaExpedientes
              ...
              loading={loading}
              onCargarDocumento={handleDocumentoCargado}
            />
          )}
          {vistaActiva === 'estadisticas' && (
            <VistaEstadisticas expedientes={expedientes} />
          )}
        </motion.div>
      </AnimatePresence>
    </div>
...
interface VistaExpedientesProps {
  ...
  loading: boolean;
  onCargarDocumento: (expedienteId: string, nuevoDoc: Documento) => void;
}

function VistaExpedientes({ expedientes, loading, onCargarDocumento }: VistaExpedientesProps) {
  ...
  const [docParaVer, setDocParaVer] = useState<{ url: string; nombre: string; tipo: string } | null>(null);
  ...
  const handleDescargarDocumento = (doc: Documento, e: React.MouseEvent) => {
    ...
    if (doc.file && ((doc.file as any) instanceof window.File || (doc.file as any) instanceof Blob)) {
      // Descarga real de archivo en memoria
      const url = URL.createObjectURL(doc.file as any);
      ...
      toast.info('Descarga no disponible', {
        description: `El archivo "${doc.nombre}" simulado no persiste tras recargar.`,
        duration: 3000,
      });
    }
  };
  ...
  const handleVerDocumento = (doc: Documento) => {
    if (doc.file && ((doc.file as any) instanceof window.File || (doc.file as any) instanceof Blob)) {
      const url = URL.createObjectURL(doc.file as any);
      setDocParaVer({ url, nombre: doc.nombre, tipo: doc.tipoArchivo });
      setVisorAbierto(true);
    } else {
      toast.info('Visualización no disponible', {
        description: `El archivo "${doc.nombre}" simulado no persiste tras recargar.`,
        duration: 3000,
      });
    }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-gray-500">Cargando expedientes del Sistema de Gestión Legal...</p>
      </div>
    );
  }
      ...
      <div className="space-y-4">
        {expedientesFiltrados.map((expediente) => (
          <CardExpediente
            ...
            onDocumentUpload={onCargarDocumento}
        ...
        {expedientesFiltrados.length === 0 && (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
            ...
            <p className="text-gray-500 text-sm">
              No hay expedientes creados o no concuerdan con la búsqueda.
            </p>
          </div>
        )}
      </div>
...
interface CardExpedienteProps {
  ...
  expandido: boolean;
  onToggleExpand: () => void;
  onDocumentUpload: (expedienteId: string, doc: Documento) => void;
  onVerDocumento: (doc: Documento) => void;
}

function CardExpediente({ expediente, expandido, onToggleExpand, onDocumentUpload, onVerDocumento }: CardExpedienteProps) {
  ...
  const tipoProcesoConfig = {
    ...
    OTRO: { label: 'Otro', icon: FileIcon, color: '#6B7280' }
  };
    ...
    <>
      ...
      {modalCargar && (
        <ModalCargarDocumento
          ...
          onUploadSuccess={(doc) => onDocumentUpload(expediente.id, doc)}
        />
      )}
    </>
  );
...
function CarpetaTipoDocumento({ tipoDocumento, documentos, icon, onVerDocumento }: CarpetaTipoDocumentoProps) {
  ...
  const handleDescargarDocumento = (doc: Documento, e: React.MouseEvent) => {
    ...
    if (doc.file && (doc.file instanceof window.File || doc.file instanceof Blob)) {
      // Descarga real de archivo en memoria
            ...
            <div className="p-3 sm:p-4 bg-white bg-opacity-50">
              {documentos.length === 0 ? (
                <div className="text-center py-6 sm:py-8 text-xs sm:text-sm opacity-60">
                  <FileIcon className="w-6 h-6 sm:w-8 sm:h-8 mx-auto mb-2 opacity-40" />
...
function ModalCargarDocumento({
  ...
  expediente: Expediente;
  onClose: () => void;
  onUploadSuccess: (doc: Documento) => void;
}) {
  ...
  const handleCargar = async () => {
    ...
    try {
      toast.loading('Cargando documento...', { id: 'upload-doc' });

      // Intentar subir usando el servicio si no es un ID mock conocido que fallaría
      if (!expediente.id.startsWith('exp-') && (legalService as any).crearDocumento) {
        await (legalService as any).crearDocumento({
          expedienteId: expediente.id,
          nombre: archivoSeleccionado.name,
          tipo: tipoDocumento,
          file: archivoSeleccionado,
          descripcion: `Carga desde Módulo Expedientes - ${tipoDocumento}`
        });
      } else {
        // Simular delay para mocks
      ...
      const nuevoDoc: Documento = {
        ...
        nombre: archivoSeleccionado.name,
        tipo: tipoDocumento,
        tipoArchivo: archivoSeleccionado.name.split('.').pop()?.toUpperCase() || 'FILE',
        tamanio: `${(archivoSeleccionado.size / 1024 / 1024).toFixed(2)} MB`,
        fechaCreacion: new Date().toISOString().split('T')[0],
        autor: 'Usuario Actual',
        file: archivoSeleccionado // Guardamos el archivo real
      };
      ...
      onUploadSuccess(nuevoDoc);
      onClose();
    } catch (error) {
      console.error('Error uploading:', error);
      // Fallback: Simulamos éxito localmente incluso si falla el servicio real
      const nuevoDocMock: Documento = {
        id: `doc-err-${Date.now()}`,
        nombre: archivoSeleccionado.name,
        tipo: tipoDocumento,
        tipoArchivo: archivoSeleccionado.name.split('.').pop()?.toUpperCase() || 'FILE',
        tamanio: `${(archivoSeleccionado.size / 1024 / 1024).toFixed(2)} MB`,
        fechaCreacion: new Date().toISOString().split('T')[0],
        autor: 'Usuario Actual (Local)',
        file: archivoSeleccionado
      };

      toast.success('Documento cargado exitosamente (Modo Local)', {
        id: 'upload-doc',
        description: `Agregado al expediente ${expediente.radicado}`,
        duration: 3000
      });

      onUploadSuccess(nuevoDocMock);
      onClose();
    }
  };
      ...
      <div className="space-y-4">
        <div>
          ...
          <div
            ...
            className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-[#003DA5] transition-colors cursor-pointer"
          >
            <Upload className="w-8 h-8 text-gray-400 mx-auto mb-2" />
            <p className="text-sm text-gray-600">
              {archivoSeleccionado ? (
                <span className="text-[#003DA5] font-medium">{archivoSeleccionado.name}</span>
              ) : (
                <>Arrastra un archivo o <span className="text-[#003DA5] font-medium">explora</span></>
              )}
            </p>
            <p className="text-xs text-gray-500 mt-1">PDF, DOCX, XLSX, ZIP (Máx. 10 MB)</p>
          </div>
        </div>
      </div>
