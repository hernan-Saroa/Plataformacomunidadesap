...
import { Users, Shield, Activity, FolderOpen, BarChart3, FileText, ClipboardList, Settings, TrendingUp, GraduationCap, ChevronLeft, ChevronRight, ChevronDown, ChevronUp, Zap, MessageSquare, Briefcase, Calendar, Newspaper, Megaphone, CheckCircle, Award, UserPlus, FolderKanban, BookOpen, FileCheck, Building2, Scale, Layout } from 'lucide-react';
...
export function SidebarPremium({ isOpen, currentModule, onModuleChange, onClose, isCollapsed = false, onToggleCollapse, forceCollapse, userRole, certificatesPendingCount = 0, restrictedMode }: SidebarProps) {
  ...
  const renderMenuWithSubmenu = (
    ...
    const isActive = currentModule === module || submenuItems?.some(item => currentModule === item.module);
          ...
          {hasSubmenu && isExpanded && (
            ...
            >
              <div className="ml-4 pl-3 border-l-2 border-white/10 mt-1.5 space-y-0.5">
                {submenuItems.map((item, idx) => {
                  const isSubmenuActive = currentModule === item.module;
                  return (
                    <motion.button
                      key={idx}
