...
// ════════════════════════════════════════════════════════════════════════════
// LOGOS ESAP FIJOS - NO MODIFICAR
// ════════════════════════════════════════════════════════════════════════════

// Logo completo ESAP (blanco sobre fondo azul) - para sidebar desplegado
import logoESAPCompleto from 'figma:asset/bf33c0f2d5f03ef0d7baf88a705a5a66362cd8c4.png';

// Isotipo ESAP (solo el círculo) - para sidebar colapsado
import isotipoESAP from 'figma:asset/4edbb7f080cc9b804243d1bbb6ad1f6cd9d2f3f7.png';

// Logo completo ESAP - para sidebar desplegado
const ESAPLogoWhite = () => (
  <img
    src={logoESAPCompleto}
    alt="ESAP - Escuela Superior de Administración Pública"
    className="h-10 w-auto mx-auto object-contain drop-shadow-lg"
  />
);

// Isotipo ESAP oficial (PNG) - para sidebar colapsado
const IsotipoESAPWhite = () => (
  <img
    src={isotipoESAP}
    alt="ESAP Isotipo"
...
// Logo pequeño para footer colapsado
const ESAPLogoSmall = () => (
  <img
    src={isotipoESAP}
    alt="ESAP"
...
export function SidebarPremium({ isOpen, currentModule, currentSidebarModule, onModuleChange, onClose, isCollapsed = false, onToggleCollapse, forceCollapse, userRole, userEmail, certificatesPendingCount = 0, restrictedMode }: SidebarProps) {
                  ...
                  {submenuItems.map((item, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-2.5 transition-all duration-200 ${isSubmenuActive
  ...
  return (
    <>
      {/* 📱 OVERLAY MOBILE con Blur Premium */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            initial={{ opacity: 0 }}
      ...
      {/* 🎨 SIDEBAR RESPONSIVE - Mobile First + CSS Variables */}
      <motion.aside
        ...
        className={`
          fixed md:static left-0 top-0 h-screen z-[100] flex flex-col
          transition-all duration-300
        `}
        style={{
          // ✅ Mobile First: Width con CSS variables
          width: effectiveCollapsed
            ? 'var(--sidebar-collapsed, 80px)'
            : 'var(--sidebar-mobile, 280px)',
            ...
            {!effectiveCollapsed ? (
              ...
              >
                <div className="w-8 h-8" title="@Esap 2026 - Todos los derechos reservados">
                  <ESAPLogoSmall className="w-full h-full object-contain drop-shadow-lg" />
                </div>
              </motion.div>
            )}
          </AnimatePresence>
