...
const CARPETAS_MOCK: Carpeta[] = [
  { id: 'c1', nombre: 'Informes de Ley', descripcion: '16 informes normativos obligatorios', parent: null, color: 'indigo', documentosCount: 24 },
  { id: 'c2', nombre: 'Planes Anuales', descripcion: 'Planes anuales de auditoría', parent: null, color: 'blue', documentosCount: 8 },
  { id: 'c3', nombre: 'Auditorías 2025', descripcion: 'Documentos de auditorías del año', parent: null, color: 'purple', documentosCount: 156 },
  { id: 'c4', nombre: 'Planes de Mejoramiento', descripcion: 'Planes y seguimientos', parent: null, color: 'emerald', documentosCount: 42 },
  { id: 'c5', nombre: 'Evidencias', descripcion: 'Evidencias de auditorías', parent: null, color: 'yellow', documentosCount: 312 },
  { id: 'c6', nombre: 'Actas', descripcion: 'Actas de reuniones', parent: null, color: 'orange', documentosCount: 18 }
];
...
const DOCUMENTOS_MOCK: Documento[] = [
  ...
  },
  {
    ...
    etiquetas: ['Evidencia', 'Plan Mejoramiento', 'Seguimiento']
  },
  {
    id: 'd4',
...
export const GestionDocumentalModule: React.FC = () => {
  ...
  const documentosFiltrados = useMemo(() => {
    ...
    if (carpetaSeleccionada) {
      docs = docs.filter(d => d.carpeta === carpetaSeleccionada);
    }
    ...
    if (busqueda.trim()) {
      ...
      docs = docs.filter(d =>
        ...
        d.etiquetas.some(e => e.toLowerCase().includes(termino))
      );
    }
  ...
  const estadisticas = useMemo(() => {
    ...
    const totalTamano = DOCUMENTOS_MOCK.reduce((sum, d) => sum + d.tamano, 0);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Repositorio centralizado de documentos de Control Interno</p>
                </div>
              </div>
            </div>

            <div className="flex gap-2">
              <ButtonSIGL variant="default" onClick={() => setModalNuevaCarpeta(true)}>
                <FolderPlus className="w-4 h-4" />
                Nueva Carpeta
              </ButtonSIGL>
              <ButtonSIGL variant="primary" onClick={() => setModalUpload(true)}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <CardSIGL>
            <div className="p-6">
              ...
              <div className="text-3xl font-bold text-gray-900 mb-1">{estadisticas.totalDocumentos}</div>
          ...
          <CardSIGL>
            <div className="p-6">
              ...
              <div className="text-3xl font-bold text-gray-900 mb-1">{estadisticas.totalCarpetas}</div>
          ...
          <CardSIGL>
            <div className="p-6">
              ...
              <div className="text-3xl font-bold text-gray-900 mb-1">{formatBytes(estadisticas.totalTamano)}</div>
          ...
          <CardSIGL>
            <div className="p-6">
              ...
              <div className="text-3xl font-bold text-gray-900 mb-1">{estadisticas.documentosHoy}</div>
              <div className="text-sm text-gray-600">Subidos Hoy</div>
            </div>
          </CardSIGL>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-4">
          <div className="flex items-center gap-2">
            <ButtonSIGL
              variant={vistaActual === 'CARPETAS' ? 'primary' : 'default'}
              onClick={() => { setVistaActual('CARPETAS'); setCarpetaSeleccionada(null); }}
            >
              <FolderOpen className="w-4 h-4" />
              Carpetas
            </ButtonSIGL>
            <ButtonSIGL
              variant={vistaActual === 'RECIENTES' ? 'primary' : 'default'}
              onClick={() => { setVistaActual('RECIENTES'); setCarpetaSeleccionada(null); }}
            >
        ...
        {vistaActual === 'CARPETAS' && carpetaSeleccionada && (
          <VistaDocumentos
            ...
            carpeta={CARPETAS_MOCK.find(c => c.id === carpetaSeleccionada)!}
        ...
        {/* MODAL NUEVA CARPETA */}
        {modalNuevaCarpeta && (
          <ModalNuevaCarpeta onClose={() => setModalNuevaCarpeta(false)} />
        )}
      </div>
    </div>
  );
          ...
          <CardSIGL>
            ...
            >
              <div className="flex items-start gap-4">
                <div className={`w-14 h-14 bg-${carpeta.color}-100 rounded-xl flex items-center justify-center flex-shrink-0`}>
                  <FolderOpen className={`w-8 h-8 text-${carpeta.color}-600`} />
                </div>

                <div className="flex-1">
                  <h3 className="font-semibold text-gray-900 mb-1">{carpeta.nombre}</h3>
                  <p className="text-sm text-gray-600 mb-3">{carpeta.descripcion}</p>
                  <div className="flex items-center gap-2">
...
const VistaDocumentos: React.FC<{
  ...
  return (
    <div className="space-y-4">
      {/* Breadcrumb */}
      <div className="flex items-center gap-2">
        <ButtonSIGL variant="default" onClick={onVolver}>
          ← Volver
        </ButtonSIGL>
        <span className="text-gray-400">/</span>
        <span className="font-semibold text-gray-900">{carpeta.nombre}</span>
      </div>
        ...
        {documentos.length === 0 && (
          <CardSIGL>
            <div className="p-12 text-center">
              ...
              <p className="text-gray-600">Esta carpeta está vacía</p>
            </div>
          </CardSIGL>
        )}
      </div>
...
const VistaRecientes: React.FC<{ documentos: Documento[] }> = ({ documentos }) => {
  return (
    <div className="space-y-2">
      {documentos.map((doc, index) => (
        <motion.div
                ...
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-600">
                      <span>{formatBytes(doc.tamano)}</span>
                      <span>•</span>
                      <span>{formatFechaRelativa(doc.fechaCreacion)}</span>
                      <span>•</span>
                      <span>{doc.creadoPor}</span>
                    </div>
                  </div>
...
const ModalUpload: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  const [archivo, setArchivo] = useState<File | null>(null);
  const [carpeta, setCarpeta] = useState('');

  return (
    <ModalSIGL isOpen={true} onClose={onClose} title="Subir Documento" size="medium">
      <div className="space-y-4">
        <div>
...
const ModalNuevaCarpeta: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  const [nombre, setNombre] = useState('');
  const [descripcion, setDescripcion] = useState('');

  return (
    <ModalSIGL isOpen={true} onClose={onClose} title="Nueva Carpeta" size="medium">
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
