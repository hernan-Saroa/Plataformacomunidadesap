...
/** Extrae un nombre legible de auditorLider/miembro (puede ser string u objeto). */
...
function VistaMes({ fecha, auditorias, onSeleccionar }: VistaMesProps) {
  ...
  // Estado para rastrear qué días están expandidos
    ...
    >
      ...
      <div className="grid grid-cols-7 gap-2">
        {diasCalendario.map((dia, idx) => {
          ...
          // Determinar cuántas auditorías mostrar
          const maxAuditorias = estaExpandido ? auditoriasDelDia.length : 4;
          const auditoriasAMostrar = auditoriasDelDia.slice(0, maxAuditorias);

          return (
            <div
              key={idx}
              className={`rounded-lg border-2 p-2 min-h-[90px] transition-shadow ${
                esFestivoDia && festivoStyle
                  ? festivoStyle.cardClass
                  : 'bg-white ' + (esHoy ? 'border-[#F57C00] ring-2 ring-[#F57C00]/30' : 'border-gray-200')
              }`}
