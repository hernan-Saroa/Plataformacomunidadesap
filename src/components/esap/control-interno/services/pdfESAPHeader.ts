...
/**
 * Obtiene la fecha actual en formato institucional (ej: '01/Abr/2026')
...
export interface ConfiguracionDocumento {
  codigo: string;                  // ej: 'EM-PT-004', 'EM-FO-003'
  version: number;                 // ej: 3
  ...
  documentoReferencia?: string;    // ej: 'Procedimiento Auditorías Internas basadas en riesgos EM-PT-004'
}
...
export function dibujarEncabezadoInstitucional(
  ...
  try {
    ...
    if (config.documentoReferencia) {
      ...
      doc.setFont('helvetica', 'normal');
      doc.text(config.documentoReferencia, logoX + 44, yPos + 4);
      yPos += alturaRef;
    }
...
export function dibujarPieInstitucional(
  ...
  if (incluirContacto) {
    ...
    doc.text('Correo Electrónico: ventanillaunica@esap.edu.co', margin, yPie + 6);
    doc.text('www.esap.edu.co', margin, yPie + 9);
  }

  // Número de página (derecha)
  const paginaTexto = totalPaginas
    ? `Página ${numeroPagina} | ${totalPaginas}`
    : `Página ${numeroPagina}`;
  doc.setFont('helvetica', 'normal');
  doc.text(paginaTexto, pageWidth - margin, yPie + 9, { align: 'right' });
}
...
export const DOCUMENTOS_PREDEFINIDOS = {
  ...
  INFORME_AUDITORIA_OCI: {
    ...
    version: 2,
    fecha: obtenerFechaActual(),
    titulo: 'INFORME DE AUDITORÍA INTERNA OCI',
    proceso: 'EVALUACIÓN CONTROL Y MEJORA',
