 ...
 * Navegación: Landing, Login, Portal Transaccional, Backoffice Administrativo
 * Features: Persistencia de sesión, Auto-logout (15 min), Alertas de inactividad
 * Actualizado: Sistema de Plantillas de Autos simplificado FUNCIONANDO
 */
...
export default function App() {
  ...
  const [
    ...
    setMostrarAlertaInactividad,
  ] = useState(false);

  const timerInactividadRef = useRef<NodeJS.Timeout | null>(
    null,
  );
  ...
  useEffect(() => {
    if (
      usuarioActual &&
      (vistaActual === "portal" || vistaActual === "backoffice")
    ) {
      const sesion: SesionGuardada = {
  ...
  const resetearTimerInactividad = useCallback(() => {
    ...
    timerAlertaRef.current = setTimeout(() => {
      ...
      toast.warning("⚠️ Inactividad detectada", {
        description:
          "Tu sesión se cerrará en 1 minuto por seguridad",
        duration: 10000,
      });
  ...
  const handleLoginExitoso = (usuario: Usuario) => {
    ...
    if (usuario.tipo === "externo") {
      ...
      toast.success(
        "¡Bienvenido al Backoffice Administrativo!",
        {
          description: `Hola ${usuario.nombre}`,
        },
      );
            ...
            <div className="mb-6">
              <div className="flex items-center gap-3 p-4 bg-orange-50 border border-orange-200 rounded-xl mb-4">
                ...
                <p className="text-sm text-gray-700">
                  Has estado{" "}
                  <strong>inactivo durante 14 minutos</strong>.
