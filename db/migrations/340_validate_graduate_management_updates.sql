-- Protege las actualizaciones realizadas desde Gestion de Graduados.
-- No modifica registros historicos ni afecta inserciones de integraciones.
-- Cada regla se evalua solo cuando el campo correspondiente cambia.
...
BEGIN
  ...
  IF NEW.num_registro IS DISTINCT FROM OLD.num_registro
     AND NEW.num_registro IS NOT NULL
     AND NEW.num_registro !~ '^[0-9]{1,20}$' THEN
    RAISE EXCEPTION
      'El numero de registro debe contener entre 1 y 20 caracteres numericos.'
      USING ERRCODE = '23514';
  END IF;

  IF NEW.num_folio IS DISTINCT FROM OLD.num_folio
     AND NEW.num_folio IS NOT NULL
     AND NEW.num_folio !~ '^[0-9]{1,10}$' THEN
    RAISE EXCEPTION
      'El numero de folio debe contener entre 1 y 10 caracteres numericos.'
      USING ERRCODE = '23514';
  END IF;

  IF NEW.num_libro IS DISTINCT FROM OLD.num_libro
     AND NEW.num_libro IS NOT NULL
     AND NEW.num_libro !~ '^[0-9]{1,10}$' THEN
    RAISE EXCEPTION
      'El numero de libro debe contener entre 1 y 10 caracteres numericos.'
      USING ERRCODE = '23514';
  END IF;
...
DROP TRIGGER IF EXISTS trg_validate_graduate_management_update
  ON academic_registration.graduates;

CREATE TRIGGER trg_validate_graduate_management_update
BEFORE UPDATE OF
  full_name,
  first_name,
  last_name,
...
ON academic_registration.graduates
FOR EACH ROW
EXECUTE FUNCTION academic_registration.validate_graduate_management_update();

COMMENT ON FUNCTION academic_registration.validate_graduate_management_update()
  IS 'Valida los campos editables de Gestion de Graduados solo cuando cambian.';

COMMENT ON TRIGGER trg_validate_graduate_management_update
  ON academic_registration.graduates
  IS 'Aplica longitudes, formatos y trim a las actualizaciones de datos del graduado.';
