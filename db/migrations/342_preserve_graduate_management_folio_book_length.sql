-- Conserva el rango historico mas amplio de Gestion de Graduados:
-- folio y libro admitian hasta 12 caracteres numericos antes de la matriz.
-- Solicitudes de Revision conserva el maximo de 10 definido para ese flujo.
...
BEGIN
  ...
  IF NEW.num_folio IS DISTINCT FROM OLD.num_folio
     AND (
       ...
       OR NEW.num_folio !~ '^[0-9]{1,12}$'
     ) THEN
    RAISE EXCEPTION
      'El numero de folio es obligatorio y debe contener entre 1 y 12 caracteres numericos.'
      USING ERRCODE = '23514';
  END IF;
  ...
  IF NEW.num_libro IS DISTINCT FROM OLD.num_libro
     AND (
       ...
       OR NEW.num_libro !~ '^[0-9]{1,12}$'
     ) THEN
    RAISE EXCEPTION
      'El numero de libro es obligatorio y debe contener entre 1 y 12 caracteres numericos.'
      USING ERRCODE = '23514';
  END IF;
...
COMMENT ON FUNCTION academic_registration.validate_graduate_management_update()
  IS 'Valida Gestion de Graduados, conservando folio y libro de hasta 12 caracteres.';
