...
echo -e "${MAGENTA}========================================${NC}"
echo -e "${MAGENTA}  ESAP SuperApp - Entorno PRE          ${NC}"
echo -e "${MAGENTA}========================================${NC}"
...
if [ -f .env.pre ]; then
    ...
    echo -e "${YELLOW}cp .env.example .env.pre${NC}"
    exit 1
fi
...
usage() {
    ...
    echo "  clean-safe - Limpieza segura Docker (preserva volúmenes de BD PRE)"
...
# Comando: up
cmd_up() {
    echo -e "${GREEN}Iniciando servicios PRE...${NC}"
    compose_env up -d
    restart_frontend_nginx
    echo -e "${GREEN}Servicios PRE iniciados exitosamente${NC}"
...
# Comando: down
cmd_down() {
    echo -e "${YELLOW}Deteniendo servicios PRE...${NC}"
    compose_env down
    echo -e "${GREEN}Servicios PRE detenidos${NC}"
}

# Comando: restart
cmd_restart() {
    echo -e "${YELLOW}Reiniciando servicios PRE...${NC}"
    compose_env restart
    restart_frontend_nginx
    echo -e "${GREEN}Servicios PRE reiniciados${NC}"
}

# Comando: rebuild
cmd_rebuild() {
    echo -e "${YELLOW}Reconstruyendo servicios PRE (sin detener la versión actual)...${NC}"
...
cmd_rebuild_all_mfe() {
    ...
    echo -e "${YELLOW}Reconstruyendo backend + gateway + shell + todos los MFEs PRE...${NC}"
    echo -e "${YELLOW}La aplicación seguirá disponible mientras termina el build.${NC}"
    echo -e "${YELLOW}Modo rápido PRE: build frontend en host + empaquetado Nginx liviano.${NC}"
    ...
    cmd_db_migrate "global" || true
    restart_frontend_nginx
    echo -e "${GREEN}App completa PRE publicada: microservicios + microfrontends.${NC}"
}
...
# Comando: status
cmd_status() {
    echo -e "${GREEN}Estado de los servicios PRE:${NC}"
    compose_env ps
}
...
# Comando: clean-safe (preserva datos de DB PRE)
cmd_clean_safe() {
    echo -e "${YELLOW}Limpieza segura Docker PRE (sin borrar volúmenes de BD)...${NC}"
    ...
    # Eliminar solo volúmenes huérfanos, excepto los protegidos de BD PRE
    while IFS= read -r v; do
        [ -z "$v" ] && continue
        case "$v" in
            esap-pgdata-pre|codigosuperappesap_pgdata-pre)
    ...
    echo -e "${GREEN}Limpieza segura PRE completada${NC}"
...
# Comando: db-backup
cmd_db_backup() {
    BACKUP_FILE="backup_esap_pre_$(date +%Y%m%d_%H%M%S).sql"
    echo -e "${YELLOW}Creando backup de la base de datos PRE...${NC}"
    docker exec superapp-db-pre pg_dump -U postgres esap_db > "./db/$BACKUP_FILE"
...
cmd_db_migrate() {
    ...
    echo -e "${YELLOW}Ejecutando migraciones de base de datos PRE...${NC}"
    if [ -n "$target_service" ] && [ "$target_service" != "all" ]; then
        echo -e "${MAGENTA}Filtro aplicado: microservicio ${target_service}${NC}"
    fi

    # Verificar que el contenedor de la base de datos está corriendo
    if ! docker ps --format '{{.Names}}' | grep -q "superapp-db-pre"; then
        echo -e "${RED}Error: El contenedor superapp-db-pre no está corriendo${NC}"
        return 1
    fi

    # Preparar carpeta temporal limpia en el contenedor
    docker exec superapp-db-pre rm -rf /tmp/migrations
    docker exec superapp-db-pre mkdir -p /tmp/migrations
    ...
    if [ -n "$target_service" ] && [ "$target_service" != "all" ]; then
        if [ "$target_service" = "global" ] || [ "$target_service" = "root" ]; then
            ...
            if [ -d "./db/migrations" ]; then
                docker cp ./db/migrations/. superapp-db-pre:/tmp/migrations/global/
                docker exec superapp-db-pre sh -c "find /tmp/migrations/global -mindepth 2 -type f -delete 2>/dev/null || true"
                found_migrations=1
            fi
        else
            ...
            for dir in "${possible_dirs[@]}"; do
                if [ -d "./backend/$dir/db/migrations" ] && [ -n "$(find "./backend/$dir/db/migrations" -maxdepth 2 -type f -name '*.sql' 2>/dev/null)" ]; then
                    ...
                    docker exec superapp-db-pre mkdir -p "/tmp/migrations/backend-$dir"
                    docker cp "./backend/$dir/db/migrations/." "superapp-db-pre:/tmp/migrations/backend-$dir/"
                    found_migrations=1
                fi
                if [ -d "./db/migrations/$dir" ] && [ -n "$(find "./db/migrations/$dir" -maxdepth 2 -type f -name '*.sql' 2>/dev/null)" ]; then
                    ...
                    docker exec superapp-db-pre mkdir -p "/tmp/migrations/db-$dir"
                    docker cp "./db/migrations/$dir/." "superapp-db-pre:/tmp/migrations/db-$dir/"
                    found_migrations=1
                fi
            done
        ...
        for svc_dir in ./backend/*/db/migrations; do
            if [ -d "$svc_dir" ] && [ -n "$(find "$svc_dir" -maxdepth 2 -type f -name '*.sql' 2>/dev/null)" ]; then
                ...
                docker exec superapp-db-pre mkdir -p "/tmp/migrations/services/$svc_name"
                docker cp "$svc_dir/." "superapp-db-pre:/tmp/migrations/services/$svc_name/"
                found_migrations=1
            fi
        done
    fi
    ...
    MIGRATION_FILES=$(docker exec superapp-db-pre sh -c "find /tmp/migrations -type f -name '*.sql' ! -path '*/old/*' ! -path '*/archive/*' ! -path '*/.*' 2>/dev/null | sort")
    ...
    docker exec superapp-db-pre psql -U postgres -d esap_db -c "CREATE TABLE IF NOT EXISTS auth.migrations_db_log (filename TEXT PRIMARY KEY, executed_at TIMESTAMP WITHOUT TIME ZONE DEFAULT now());" >/dev/null
    MIGRATIONS_APPLIED=$(docker exec superapp-db-pre psql -U postgres -d esap_db -At -c "SELECT filename FROM auth.migrations_db_log" 2>/dev/null)
    ...
    for file in $MIGRATION_FILES; do
        ...
        if docker exec superapp-db-pre psql -U postgres -d esap_db -f "$file" 2>&1; then
            ...
            docker exec superapp-db-pre psql -U postgres -d esap_db -c "INSERT INTO auth.migrations_db_log (filename) VALUES ('$escaped_relpath') ON CONFLICT (filename) DO NOTHING;" >/dev/null
            docker exec superapp-db-pre psql -U postgres -d esap_db -c "INSERT INTO auth.migrations_db_log (filename) VALUES ('$escaped_filename') ON CONFLICT (filename) DO NOTHING;" >/dev/null
        else
...
cmd_db_reset() {
    echo -e "${RED}⚠️  ADVERTENCIA: Esto eliminará TODOS los datos de la base de datos PRE${NC}"
    ...
    docker volume rm esap-pgdata-pre 2>/dev/null || docker volume rm codigosuperappesap_pgdata-pre 2>/dev/null || true

    echo -e "${YELLOW}Reiniciando servicios (la DB se recreará)...${NC}"
    compose_env up -d

    echo -e "${GREEN}Base de datos PRE reiniciada con datos iniciales${NC}"
}
