...
echo -e "${MAGENTA}========================================${NC}"
echo -e "${MAGENTA}  ESAP SuperApp - Entorno PROD          ${NC}"
echo -e "${MAGENTA}========================================${NC}"
...
ensure_docker_disk_space() {
    ...
    echo -e "${YELLOW}Ejecuta en el server PROD:${NC}"
    echo -e "${YELLOW}  ./deploy.prod.sh clean-safe${NC}"
...
usage() {
    ...
    echo "  clean-safe - Limpieza segura Docker (preserva volúmenes de BD PROD)"
...
# Comando: rebuild
cmd_rebuild() {
    echo -e "${YELLOW}Reconstruyendo servicios PROD (sin detener la versión actual)...${NC}"
    ...
    echo -e "${GREEN}Nueva versión publicada. Servicios PROD reconstruidos y reiniciados.${NC}"
}
...
cmd_rebuild_fresh() {
    ...
    echo -e "${YELLOW}Reconstrucción fresca PROD: se detendrá el stack y habrá downtime.${NC}"
    ...
    echo -e "${YELLOW}Deteniendo stack PROD sin borrar volúmenes...${NC}"
    ...
    echo -e "${YELLOW}Reconstruyendo todo el stack PROD desde cero...${NC}"
    ...
    restart_ssl_proxy
    echo -e "${GREEN}Reconstrucción fresca PROD completada sin borrar la DB.${NC}"
}
...
cmd_rebuild_select() {
    ...
    echo ""
    echo -e "${GREEN}Selecciona un servicio PROD para rebuild:${NC}"
...
# Comando: clean-safe (preserva datos de DB PROD)
cmd_clean_safe() {
    echo -e "${YELLOW}Limpieza segura Docker PROD (sin borrar volúmenes de BD)...${NC}"
    ...
    # Eliminar solo volúmenes huérfanos, excepto los protegidos de BD PROD
    while IFS= read -r v; do
        [ -z "$v" ] && continue
        case "$v" in
            esap-pgdata-prod|codigosuperappesap_pgdata-prod)
    ...
    echo -e "${GREEN}Limpieza segura PROD completada${NC}"
...
# Comando: db-backup
cmd_db_backup() {
    BACKUP_FILE="backup_esap_prod_$(date +%Y%m%d_%H%M%S).sql"
    echo -e "${YELLOW}Creando backup de la base de datos PROD...${NC}"
    docker exec superapp-db-prod pg_dump -U postgres esap_db > "./db/$BACKUP_FILE"
...
cmd_db_migrate() {
    ...
    echo -e "${YELLOW}Ejecutando migraciones de base de datos PROD...${NC}"
    ...
    if ! docker ps --format '{{.Names}}' | grep -q "superapp-db-prod"; then
        echo -e "${RED}Error: El contenedor superapp-db-prod no está corriendo${NC}"
        return 1
    fi

    # Preparar carpeta temporal limpia en el contenedor
    docker exec superapp-db-prod rm -rf /tmp/migrations
    docker exec superapp-db-prod mkdir -p /tmp/migrations
    ...
    if [ -n "$target_service" ] && [ "$target_service" != "all" ]; then
        if [ "$target_service" = "global" ] || [ "$target_service" = "root" ]; then
            ...
            if [ -d "./db/migrations" ]; then
                docker cp ./db/migrations/. superapp-db-prod:/tmp/migrations/global/
                docker exec superapp-db-prod sh -c "find /tmp/migrations/global -mindepth 2 -type f -delete 2>/dev/null || true"
                found_migrations=1
            fi
        else
            ...
            for dir in "${possible_dirs[@]}"; do
                if [ -d "./backend/$dir/db/migrations" ] && [ -n "$(find "./backend/$dir/db/migrations" -maxdepth 2 -type f -name '*.sql' 2>/dev/null)" ]; then
                    ...
                    docker exec superapp-db-prod mkdir -p "/tmp/migrations/backend-$dir"
                    docker cp "./backend/$dir/db/migrations/." "superapp-db-prod:/tmp/migrations/backend-$dir/"
                    found_migrations=1
                fi
                if [ -d "./db/migrations/$dir" ] && [ -n "$(find "./db/migrations/$dir" -maxdepth 2 -type f -name '*.sql' 2>/dev/null)" ]; then
                    ...
                    docker exec superapp-db-prod mkdir -p "/tmp/migrations/db-$dir"
                    docker cp "./db/migrations/$dir/." "superapp-db-prod:/tmp/migrations/db-$dir/"
                    found_migrations=1
                fi
            done
        ...
        for svc_dir in ./backend/*/db/migrations; do
            if [ -d "$svc_dir" ] && [ -n "$(find "$svc_dir" -maxdepth 2 -type f -name '*.sql' 2>/dev/null)" ]; then
                ...
                docker exec superapp-db-prod mkdir -p "/tmp/migrations/services/$svc_name"
                docker cp "$svc_dir/." "superapp-db-prod:/tmp/migrations/services/$svc_name/"
                found_migrations=1
            fi
        done
    fi
    ...
    docker exec superapp-db-prod psql -U postgres -d esap_db -c "CREATE TABLE IF NOT EXISTS auth.migrations_db_log (filename TEXT PRIMARY KEY, executed_at TIMESTAMP WITHOUT TIME ZONE DEFAULT now());" >/dev/null
    MIGRATIONS_APPLIED=$(docker exec superapp-db-prod psql -U postgres -d esap_db -At -c "SELECT filename FROM auth.migrations_db_log" 2>/dev/null)
...
cmd_db_reset() {
    echo -e "${RED}⚠️  ADVERTENCIA: Esto eliminará TODOS los datos de la base de datos PROD${NC}"
