...
export function SemaforoSeguimientoPAI({
  ...
  if (variant === 'badge') {
    return (
      <div
        className={`inline-flex items-center space-x-2 rounded-lg font-semibold border-2 ${color.bg} ${color.text} ${color.border} ${sizes.badge} ${className}`}
      >
...
export function TablaSemaforoPAI({ actividades, className = '' }: TablaSemaforoProps) {
  const promedioAvance =
    actividades.length > 0
      ? Math.round(actividades.reduce((sum, actividad) => sum + actividad.porcentaje, 0) / actividades.length)
      : 0;
    ...
    <div className={`bg-white rounded-xl border-2 border-gray-200 overflow-hidden ${className}`}>
      <div className="bg-gradient-to-r from-[#003DA5] to-[#2962FF] p-6 text-white">
        <div className="flex items-center justify-between">
          ...
          <SemaforoSeguimientoPAI porcentaje={promedioAvance} variant="circular" size="lg" showIcon={false} />
        </div>
      </div>
            ...
            {actividades.map((actividad, index) => (
              <tr key={actividad.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-4 text-center">
                  <SemaforoSeguimientoPAI porcentaje={actividad.porcentaje} variant="badge" size="sm" />
                </td>
                <td className="px-6 py-4 text-center">
      ...
      <div className="bg-gray-50 border-t-2 border-gray-200 p-4">
        <div className="flex items-center justify-between text-sm">
          ...
          <SemaforoSeguimientoPAI porcentaje={promedioAvance} variant="badge" size="md" />
        </div>
      </div>
    </div>
  );
