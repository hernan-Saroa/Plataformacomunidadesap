...
export default function PresupuestoInbox() {
  ...
  const [filtroEstado, setFiltroEstado] = useState<'EN_PRESUPUESTO' | 'COMPROMETIDA' | 'TODOS'>('EN_PRESUPUESTO');
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center space-x-3">
          ...
          {puedeExpedirRp && (
            <button
              ...
              className="px-4 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-700 hover:from-emerald-700 hover:to-teal-800 text-white text-xs font-bold rounded-xl shadow-md hover:shadow-lg transition-all flex items-center space-x-2"
            >
              <UploadCloud className="w-4 h-4" />
              <span>Carga Masiva de RPs</span>
            </button>
          )}
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center p-1 bg-slate-100 rounded-xl space-x-1 w-full md:w-auto">
          <button
            type="button"
            onClick={() => setFiltroEstado('EN_PRESUPUESTO')}
            className={`px-4 py-2 text-xs font-bold rounded-lg transition-all ${
              filtroEstado === 'EN_PRESUPUESTO'
                ? 'bg-white text-emerald-800 shadow-sm'
                : 'text-slate-600 hover:text-slate-900'
            }`}
          >
            Pendientes RP ({kpis.pendientesRp})
          </button>
          <button
            type="button"
                      ...
                      <td className="px-4 py-3 text-right">
                        {!estaComprometida && puedeExpedirRp ? (
                          <button
                            ...
                            className="px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-xl transition-all shadow-sm flex items-center space-x-1.5 ml-auto"
                          >
                            <FileCheck2 className="w-3.5 h-3.5" />
                            <span>Registrar RP</span>
                          </button>
                        ) : estaComprometida ? (
