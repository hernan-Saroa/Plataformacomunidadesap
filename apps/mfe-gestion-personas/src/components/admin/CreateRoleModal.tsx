...
// Import del servicio
import { localRolesService as rolesService, type Territorial, type CETAP, type ProgramaAcademico } from '../../services/api';
...
const [loadingDatosMaestros, setLoadingDatosMaestros] = useState(false);
...
export function CreateRoleModal({ isOpen, onClose, onSave }: CreateRoleModalProps) {
  ...
  const getUsuariosSimulados = () => {
    ...
    let count = 1250;
    if (selTerritorial !== 'Todas') count = Math.floor(count / territoriales.length); // dividir por número de territoriales
    if (selCetap !== 'Todos') count = Math.floor(count / 3);
    if (selPrograma !== 'Todos') count = Math.floor(count / programas.length); // dividir por número de programas
    return Math.max(count, 1); // mínimo 1 usuario
  };

  const handleTerritorialChange = (v: string) => {
    setSelTerritorial(v);
    setSelCetap('Todos'); // reset cascade
    // No necesitamos recargar CETAPs ya que ya los tenemos todos y filtramos localmente
  };
  ...
  const handleSubmit = () => {
    ...
    // Cargar datos maestros al abrir el modal
  useEffect(() => {
    if (isOpen) {
      loadDatosMaestros();
    }
  ...
  const loadDatosMaestros = async () => {
    try {
      setLoadingDatosMaestros(true);
      const [territorialesData, cetapsData, programasData] = await Promise.all([
        rolesService.getTerritoriales(),
        rolesService.getCETAPs(),
        rolesService.getProgramasAcademicos()
      ]);
      ...
      setLoadingDatosMaestros(false);
    }
  };

  // Función para obtener CETAPs filtrados por territorial
  const getCetapsForTerritorial = (territorialId: string) => {
    return cetaps.filter(cetap => cetap.territorialId === territorialId);
  };
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex-1 bg-white rounded-2xl border border-gray-200 p-5 shadow-sm space-y-4">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          disabled={selTerritorial === 'Todas' || loadingDatosMaestros}
                        ...
                        >
                          ...
                          {selTerritorial !== 'Todas' && getCetapsForTerritorial(selTerritorial).map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}
                        </select>
                      </div>
                ...
                <div className="w-80 bg-gray-50 p-6 flex flex-col border-l border-gray-100">
                  ...
                  <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm space-y-4">
                    ...
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <div className="font-bold text-gray-900 truncate max-w-[150px]" title={tipoAlcance === 'Global' ? 'Todas' : (territoriales.find(t => t.id === selTerritorial)?.nombre || selTerritorial)}>{tipoAlcance === 'Global' ? 'Todas' : (territoriales.find(t => t.id === selTerritorial)?.nombre || selTerritorial)}</div>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <div className="font-bold text-gray-900 truncate max-w-[150px]" title={tipoAlcance === 'Global' ? 'Todos' : (cetaps.find(c => c.id === selCetap)?.nombre || selCetap)}>{tipoAlcance === 'Global' ? 'Todos' : (cetaps.find(c => c.id === selCetap)?.nombre || selCetap)}</div>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <div className="font-bold text-gray-900 truncate max-w-[150px]" title={tipoAlcance === 'Global' ? 'Todos' : (programas.find(p => p.id === selPrograma)?.nombre || selPrograma)}>{tipoAlcance === 'Global' ? 'Todos' : (programas.find(p => p.id === selPrograma)?.nombre || selPrograma)}</div>
                    </div>
