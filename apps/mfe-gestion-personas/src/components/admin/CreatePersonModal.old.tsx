...
export function CreatePersonModal({ 
  ...
  // Inicializar con datos de edici├│n si existen
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    if (validateStep(step)) {
      ...
      // Simular creaci├│n (en producci├│n esto ser├¡a un API call)
      try {
        setIsSubmitting(true);
        await onCreate({
          ...formData,
          id: initialData?.id || `user-${Date.now()}`, // Generar ID ├║nico
  ...
  const stepTitles = [
    { number: 1, title: 'Datos b├ísicos de identificaci├│n' },
    { number: 2, title: 'Informaci├│n de Contacto' },
    { number: 3, title: 'Informaci├│n Institucional' }
  ];
              ...
              {step === 3 && (
                ...
                >
                  ...
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-2">
                      <UsersIcon className="w-4 h-4 inline-block mr-1.5 text-[#003DA5]" />
                      Rol en la Instituci├│n <span className="text-red-500">*</span>
                    </label>
                    <select
                      value={formData.role}
