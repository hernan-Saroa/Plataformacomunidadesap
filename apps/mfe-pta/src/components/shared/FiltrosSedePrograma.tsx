 ...
 * ✅ MIGRADO A SUPABASE - Recibe sedes y programas como props
 */
...
interface FiltrosSedeProgramaProps {
  ...
  // ✅ NUEVO: Recibir datos desde el componente padre (conectado a Supabase)
            ...
            {showSedes && (
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                  {sedesDisponibles.slice(0, 12).map((sede) => (
                    <button
                      key={sede.codigo}
      ...
      {cantidadFiltrosActivos > 0 && (
        <div className="flex flex-wrap gap-2">
          {filtros.sedes.map((codigoSede) => {
            const sede = sedesDisponibles.find((s) => s.codigo === codigoSede);
            return (
              <Badge
                key={codigoSede}
                variant="outline"
