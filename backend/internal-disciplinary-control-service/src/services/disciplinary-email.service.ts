...
export class DisciplinaryEmailService {
  ...
  /**
   * Envía un correo oficial al profesional que estaba asignado al proceso
   * cuando este es reasignado a otro profesional.
   */
  async sendReassignedFromEmail(
    to: string,
    profesionalNombre: string,
    radicadoProceso: string,
    ...
    const html = `
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            <h2 style="margin-top:0; color: #111827;">Hola, ${profesionalNombre}</h2>
            <p>Le informamos que el proceso disciplinario <strong>${radicadoProceso}</strong> ha sido reasignado a otro profesional.</p>
            
            <div class="info-box">
              <p style="margin-top:0;"><strong>Radicado del Proceso:</strong><br><span style="color: #003DA5; font-size: 18px; font-weight: 700;">${radicadoProceso}</span></p>
              <p><strong>Profesional Anterior:</strong><br>${profesionalNombre}</p>
              <p><strong>Nuevo Profesional:</strong><br>${nuevoProfesionalNombre}</p>
              ${justificacion ? `<p><strong>Justificación:</strong><br>${justificacion}</p>` : ''}
              ${observacionesJefe ? `<p><strong>Observaciones del Jefe:</strong><br>${observacionesJefe}</p>` : ''}
            </div>
    ...
    try {
      await firstValueFrom(
        this.httpService.post(`${notificationsUrl}/api/v1/emails/send`, {
          to,
          subject: `[REASIGNACIÓN] Proceso reasignado: ${radicadoProceso}`,
          html,
        }),
      );
      this.logger.log(`Email de reasignación (profesional anterior) enviado exitosamente a ${to}`);
      return true;
    } catch (error: any) {
      this.logger.error(`Error enviando email de reasignación a ${to}: ${error.message}`);
      return false;
