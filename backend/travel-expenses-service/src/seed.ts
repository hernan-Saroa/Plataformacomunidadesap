...
async function seed() {
  ...
  try {
    ...
    const existingEscalas = await dataSource
      .getRepository(EscalaViaticoEntity)
      .count();
    ...
    const existingTarifasInv = await dataSource
      .getRepository(TarifaInvestigadorEntity)
      .count();
    if (existingTarifasInv === 0) {
      await dataSource.getRepository(TarifaInvestigadorEntity).save([
        { categoriaInvestigador: 'JUNIOR', tarifaDiaria: 250000 },
    ...
    const existingRegionales = await dataSource
      .getRepository(TarifaRegionalExcepcionEntity)
      .count();
