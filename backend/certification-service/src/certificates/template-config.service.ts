...
export class TemplateConfigService {
  ...
  private getDefaultContent(templateType: string): string {
    if (templateType === 'administrador') {
      return 'Que [NOMBRE_EMPLEADO] identificado con c\u00E9dula de ciudadan\u00EDa No. [DOCUMENTO], se encuentra vinculado con la Escuela Superior de Administraci\u00F3n P\u00FAblica - ESAP mediante nombramiento [TIPO_DATO] desde el [FECHA_INICIO], desempe\u00F1ando el cargo de [CARGO] ubicado en [UBICACIÓN].<br><br><div>Que [NOMBRE_EMPLEADO] percibe mensualmente una asignaci\u00F3n salarial de [SALARIO] [SALARIO_LETRAS] pesos m/cte.<br><br></div><div>Se expide en la ciudad de Bogot\u00E1 D.C., a solicitud del interesado(a) a los [FECHA_EXPEDICION_COMPLETA].</div>';
    }
